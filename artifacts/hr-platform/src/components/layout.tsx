...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const isAiPage = location === "/" || location === "/assistente";
  ...
  return (
    <div className="flex h-screen w-full bg-background overflow-hidden">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <header className={`flex h-14 shrink-0 items-center px-5 bg-card border-b border-border z-10 sticky top-0 gap-3 ${isAiPage ? "bg-background border-transparent" : ""}`}>
          <button
            onClick={toggle}
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden p-6 md:p-8">
          <div className={`mx-auto w-full h-full ${isAiPage ? "max-w-4xl" : "max-w-7xl"}`}>
            {children}
          </div>
        </main>
      </div>
