  ...
  return (
    <div className="mb-6">
      ...
      <div className="relative h-3 rounded-full bg-muted overflow-visible mx-1">
        {/* IQR bar P25-P75 */}
        <div
          className="absolute top-0 h-full rounded-full bg-violet-200"
          style={{ left: `${p25pct}%`, width: `${p75pct - p25pct}%` }}
        />
        {/* Median tick */}
        <div
          className="absolute top-[-3px] bottom-[-3px] w-0.5 bg-violet-600 rounded-full"
      ...
      <div className="flex justify-between mt-2 text-[11px] text-muted-foreground">
        ...
        <span className="text-violet-600 font-medium">Mediana: {formatCurrency(median)}</span>
                ...
                <div className="mb-5 p-3 bg-muted/40 rounded-lg border border-border">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs text-muted-foreground">
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-2 rounded-full bg-violet-200" />
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {regionalData.regions.map((r) => {
                      ...
                      return (
                        <div
                          key={r.region}
                          className={`p-3 rounded-xl border ${isSP ? "bg-violet-600 border-violet-600 text-white" : "bg-card border-border"}`}
                        >
                          <div className="flex items-center justify-between mb-1.5">
                            <div className={`flex items-center gap-1 text-xs ${isSP ? "text-violet-200" : "text-muted-foreground"}`}>
                              <MapPin className="w-3 h-3" />
                              <span>{r.region}</span>
                            </div>
                            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${isSP ? "bg-violet-500 text-white" : "bg-muted text-foreground"}`}>
                              {r.index}
                            </span>
                          </div>
                          ...
                          <div className={`text-[11px] mt-0.5 ${isSP ? "text-violet-200" : "text-muted-foreground"}`}>
                  ...
                  <Table>
                    <TableHeader className="bg-muted/50">
                      <TableRow className="hover:bg-transparent">
                        ...
                        <TableHead className="font-semibold text-foreground text-right">P25</TableHead>
                        <TableHead className="font-semibold text-foreground text-right text-violet-700 bg-violet-50">Mediana</TableHead>
                        <TableHead className="font-semibold text-foreground text-right">P75</TableHead>
                        ...
                        salaryBands.bands.map((band) => {
                          ...
                          return (
                            <TableRow key={band.seniority} className="hover:bg-muted/20 transition-colors">
                              ...
                              <TableCell className="text-right font-bold text-violet-700 bg-violet-50">
