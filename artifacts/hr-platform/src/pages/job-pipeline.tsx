...
const FALLBACK_STAGES: PipelineStage[] = [
  ...
  { id: 2, name: "entrevista_rh", label: "Entrevista RH", color: "border-indigo-200 bg-indigo-50", position: 2, isTerminal: false, createdAt: new Date().toISOString() },
  ...
  { id: 6, name: "reprovado", label: "Reprovado", color: "border-rose-200 bg-rose-50", position: 6, isTerminal: true, createdAt: new Date().toISOString() },
];
...
export default function JobPipeline() {
  ...
  const firstStage = stages[0]?.name ?? "triagem";

  const form = useForm<z.infer<typeof createCandidateSchema>>({
    resolver: zodResolver(createCandidateSchema),
    defaultValues: { stage: firstStage, source: "linkedin" },
  });

  const onSubmit = (data: z.infer<typeof createCandidateSchema>) => {
    createCandidateMutation.mutate(
      { id: jobId, data: data as CreateCandidateBody },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: [`/api/jobs/${jobId}`] });
          setIsDialogOpen(false);
          form.reset({ stage: firstStage, source: "linkedin" });
  ...
  if (jobLoading || stagesLoading) {
    return (
      <div className="space-y-6 h-[calc(100vh-8rem)]">
        <Skeleton className="h-24 w-full rounded-2xl" />
        <div className="flex gap-4 h-full">
          {[1, 2, 3, 4, 5].map((i) => (
            <Skeleton key={i} className="w-[300px] h-full rounded-xl shrink-0" />
          ))}
        </div>
      </div>
    );
              ...
              <div className="bg-gradient-to-r from-primary to-indigo-600 p-6">
                <DialogHeader>
                  ...
                  <DialogDescription className="text-indigo-100">
                    Adicione manualmente um candidato ao funil desta vaga.
                  </DialogDescription>
                </DialogHeader>
              </div>
              <Form {...form}>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea className="rounded-xl resize-none" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden pb-4">
        <DragDropContext onDragEnd={onDragEnd}>
          <div className="flex gap-4 h-full items-start px-1">
            {stages.map((stage) => {
              const columnCandidates = columns[stage.name] ?? [];
              return (
                <div
                  key={stage.name}
                  className={`w-[320px] shrink-0 flex flex-col max-h-full rounded-2xl border ${stage.color} overflow-hidden`}
                >
